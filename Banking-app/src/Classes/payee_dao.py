...
#             dbcursor = None                
  ...
  def getPayeeList(self,user_id): 
      ...
      try: 
            sql = "select beneficiaries_account_holder,beneficiaries_account_number,beneficiaries_account_type,beneficiaries_bank from beneficiaries_details where user_id =%d" % (int(user_id))
            my_logger.info(sql)
            payee_dao_access.dbcursor.execute(sql)
            ...
            if(results!=None):
                for row in results:
                    if (row != None):
                       ...
                       msg =beneficiaries_account_number+"@"+beneficiaries_account_type +"@"+ beneficiaries_account_holder +"@" + beneficiaries_bank
